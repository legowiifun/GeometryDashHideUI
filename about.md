...
This mod is known not to work with: the players in Globed, Prism Menu's UI elements, Friends!, and a couple UI elements in the main levels (this is a bug on Node ID's end, they work fine in custom levels). Most of the ones listed here are because the mod developers did not add Node IDs to there custom nodes. 
